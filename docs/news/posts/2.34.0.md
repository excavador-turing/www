...
- **The switch's link column reads `1 Gb`.** , not `1000 Mb/s · full duplex` — which was most of the table's width and pushed it into a horizontal scrollbar inside a column. <small class="tp-tag">BMC-UI 3.30.0</small>

    ??? note "The whole entry"

        Half duplex still shows, in amber, because a gigabit port that negotiated half is a bad cable.
