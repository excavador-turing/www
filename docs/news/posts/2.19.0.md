...
date: 2026-09-09
slug: v2.19.0
description: "tpi v1.5.1 \u2192 v1.6.0: the check that caused all of this is gone."
---
...
exist.** It waited 60 s for a poll, then gave up and printed the previous listing with nothing said, indistinguishable from a current one. Polls measured on this board took 74 s, 78 s and 140 s.

    ??? note "The whole entry"

        Now four minutes, it says when it gives up, and `list` prints how old the listing is on every run — the daemon had always sent that, and the client discarded it.
