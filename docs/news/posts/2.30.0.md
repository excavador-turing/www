---
title: "The metrics say which module is armed for USB boot"
date: 2026-09-12
slug: v2.30.0
description: "Pins bmcd 2.36.3. BMC-UI stays at 3.27.0 and tpi is unchanged."
---
...
Firmware **v2.30.0**, 12 September 2026 — the first image to carry [bmcd 2.36.3](../../changelog/bmcd.md). Every item below is the repository's own changelog entry, taken apart; the whole entry is a click away under each one.
...
armed for USB boot
<small class="tp-tag">bmcd 2.36.3</small>
...
to `(node, mode, route)`.** The `type=usb` response and the metrics exposition share it, so a board cannot report one thing through the API and another on a dashboard. <small class="tp-tag">bmcd 2.36.3</small>

## Fixed

- **The board refused every attempt to resume a TLS session, fatally, and that
was the intermittent console failure.** A serial console or an API call through the fleet gateway would occasionally fail outright, on both boards, with nothing in the daemon's log and the board healthy either side of it.
