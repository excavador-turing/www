---
title: "Everything current \u2014 the daemon, the interface and the CLI"
date: 2026-09-11
slug: v2.24.0
description: "Everything current: the daemon, the interface and the CLI."
image: "captures/info.png"
---

Everything current: the daemon, the interface and the CLI.

<figure class="tp-post-figure" markdown>
![The About tab — kernel, daemon, interface and firmware, each a version you can check.](../../assets/captures/info.png){ loading=lazy }
<figcaption>The About tab — kernel, daemon, interface and firmware, each a version you can check.</figcaption>
</figure>
...
- **BMC-UI v3.21.0 → v3.23.0.** V3.22.0 stopped the daemon's messages being escaped twice — i18next escapes interpolated values and React escapes again, so a path arrived as `&#x2F;` in the page.
