...
it always did. What shrinks is the exposure machinery per board, not the
firmware.
...
Boards on different firmware are normal here and always will be, so every
reading is allowed to be absent, and a board running a daemon outside the
range the page was built against says so in a banner rather than breaking.
